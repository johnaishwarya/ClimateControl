        ...
        for (var j = 0; j < categoryData.length; j++) {
            ...
            if(categoryData[j].scoringType === 'descriptive'){
                ...
                select.addEventListener('change', function() {
                    var numericValue = parseFloat(select.value);
    
                    if (!isNaN(numericValue)) {
                        if (numericValue >= 0.0 && numericValue <= 0.2) {
                            select.value = 'low';
                        } else if (numericValue > 0.2 && numericValue <= 0.4) {
                            select.value = 'low-mid';
                        } else if (numericValue > 0.4 && numericValue <= 0.6) {
                            select.value = 'mid';
                        } else if (numericValue > 0.6 && numericValue <= 0.8) {
                            select.value = 'mid-high';
                        } else if (numericValue > 0.8 && numericValue <= 1.0) {
                            select.value = 'high';
                        }
                    }
                });
